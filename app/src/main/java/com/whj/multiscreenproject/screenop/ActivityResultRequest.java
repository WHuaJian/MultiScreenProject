...
public class ActivityResultRequest {
    ...
    public static Observable<OnActivityResultDispatcherFragment> rxOf(FragmentActivity activity) {
        return Observable
                .just(activity)
                .map(new Function<FragmentActivity, OnActivityResultDispatcherFragment>() {
                    @Override
                    public OnActivityResultDispatcherFragment apply(FragmentActivity fragmentActivity) throws Exception {
                        return getDispatcherFragment(fragmentActivity);
                    }
                });
//                .map(ActivityResultRequest::getDispatcherFragment);
    }

    public static Observable<ResultEvent> rxQuest(FragmentActivity activity, final Intent intent) {
        return Observable
                .just(activity)
                .map(new Function<FragmentActivity, OnActivityResultDispatcherFragment>() {
                    @Override
                ...
                .flatMap(new Function<OnActivityResultDispatcherFragment, ObservableSource<ResultEvent>>() {
                    @Override
                    public ObservableSource<ResultEvent> apply(OnActivityResultDispatcherFragment onActivityResultDispatcherFragment) throws Exception {
